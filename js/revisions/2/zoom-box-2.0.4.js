...
(function($){
    ...
    function loadImageReady(src,width,height,controllers,settings)
    {
        ...
        if (typeof settings.afterImageReady === "function") settings.afterImageReady(settings);
    }
    ...
    function animatePosition(controllers, settings)
    {
        ...
        if (controllers.touchdrag) {
            targetleft = curleft+controllers.touchDiffX;
            targettop = curtop+controllers.touchDiffY;
        } else {
            targetleft = ((((controllers.mouseX)*-1)/(zbobj_width))*((zbimg_width-zbobj_width)));
            targettop = ((((controllers.mouseY)*-1)/(zbobj_height))*((zbimg_height-zbobj_height)));
        }
        ...
        if (controllers.touchdrag) {
            controllers.$zoomboxImage.css('left',targetleft+'px');
            controllers.$zoomboxImage.css('top',targettop+'px');
        } else {
        ...
        controllers.touchDiffX = 0;
        controllers.touchDiffY = 0;
    }
    ...
    function create(id,controllers,settings)
    {
            ...
            if (controllers.$renderContainer && controllers.$renderContainer.length)
            {
                ...
                controllers.$mousetrap.on('mousemove',function(e){
                    ...
                    if (controllers.moveready)
                    {
                        ...
                        if (controllers.touchX !== false && controllers.touchY !== false)
                        {
                    ...
                    controllers.touchX = false;
                    controllers.touchY = false;
                }).on('mouseup',function(e){
                    e.preventDefault();
                    if (settings.imageClickClose && controllers.islive)
                    {
                        var pr = 5;
                        var zoombox_offset = controllers.$zoombox.offset();
                ...
                setTimeout(function(){
                    controllers.islive = true;
                },10);

                zb[id].push({controllers:controllers,settings:settings});
            ...
            else
            {
                ...
                $objects.each(function(){
                    ...
                    $imgs.each(function(){
                        ...
                        create(newid,controllers,q_settings);
